...
public class Agent : MonoBehaviour
{
    ...
    public void Update()
    {
        if (Input.GetMouseButtonDown(moveButton))
        {
            ...
            if (targetPos.x > MazeGeneration.instance.grid.GetLength(0) - 1 || targetPos.x < 0) { Debug.LogWarning("Target outside grid"); return; }
            if (targetPos.y > MazeGeneration.instance.grid.GetLength(1) - 1 || targetPos.y < 0) { Debug.LogWarning("Target outside grid"); return; }
